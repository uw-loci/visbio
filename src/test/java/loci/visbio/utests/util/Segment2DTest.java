...
public class Segment2DTest extends TestCase {
	...
	public void testBoundingBoxOverlapsBoundingBoxOf() {
		// intersecting segments
		assertEquals("a should overlap b", true, a
			.boundingBoxOverlapsBoundingBoxOf(b));
		assertEquals("b should overlap a", true, b
			.boundingBoxOverlapsBoundingBoxOf(a));

		// adjoining segments
		assertEquals("a should overlap c", true, a
			.boundingBoxOverlapsBoundingBoxOf(c));
		assertEquals("c should overlap a", true, c
			.boundingBoxOverlapsBoundingBoxOf(a));

		// disjoint segments whose boxes touch
		assertEquals("c should overlap b", true, c
			.boundingBoxOverlapsBoundingBoxOf(b));
		assertEquals("b should overlap c", true, b
			.boundingBoxOverlapsBoundingBoxOf(c));

		// identical segments
		// not sure about this one
		assertEquals("a should overlap a", true, a
			.boundingBoxOverlapsBoundingBoxOf(a));

		// disjoint bounding boxes
		assertEquals("a should not overlap d", false, a
			.boundingBoxOverlapsBoundingBoxOf(d));
	}

	public void testBetween() {
		final float[] ca = { (float) c.getPtA().getX(), (float) c.getPtA().getY() };
		final float[] cb = { (float) c.getPtB().getX(), (float) c.getPtB().getY() };
		final float[] aa = { (float) a.getPtA().getX(), (float) a.getPtA().getY() };
		final float[] ab = { (float) a.getPtB().getX(), (float) a.getPtB().getY() };

		assertEquals("ca should be inside aa and ab", true, MathUtil.inside(ca, aa,
			ab));
		assertEquals("cb should be inside aa and ab", false, MathUtil.inside(cb,
			aa, ab));
	}
