 ...
 */
public class TransformPanel extends JPanel implements ActionListener,
	ChangeListener, ListSelectionListener
{
	...
	public TransformPanel(final TransformHandler h) {
		super();
		handler = h;
	...
	public void addAxis(final String axis) {
		final int count = animBox.getItemCount();
		final String s = "<" + count + "> " + axis;
	...
	public int getAxisCount() {
		return animBox.getItemCount() - 1;
	}

	/** Gets axis mapped to left/right arrow keys. */
	public int getLeftRightAxis() {
		return leftRightBox.getSelectedIndex() - 1;
	}

	/** Gets axis mapped to up/down arrow keys. */
	public int getUpDownAxis() {
		return upDownBox.getSelectedIndex() - 1;
	}

	/** Updates controls to reflect current handler status. */
	public void updateControls() {
		final DataTransform data = (DataTransform) transformList.getSelectedValue();
		final int index = transformList.getSelectedIndex();
		final boolean b = data != null;
		...
		if (b) {
			final TransformLink link = handler.getLink(data);
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (cmd.equals("delayBurn")) {
			final boolean delay = ((JCheckBox) e.getSource()).isSelected();
			delayTime.setEnabled(delay);
			handler.setBurnDelay(delay ? 1000 * ((Number) delayTime.getValue())
				.intValue() : 0);
		}
		else if (cmd.equals("visible")) {
			final DataTransform data =
				(DataTransform) transformList.getSelectedValue();
			if (data != null) {
				final JCheckBox vis = (JCheckBox) e.getSource();
			...
			final DataManager dm =
				(DataManager) handler.getWindow().getVisBio().getManager(
					DataManager.class);
			final DefaultMutableTreeNode root = dm.getDataRoot();
			...
			for (int i = 0; i < children; i++) {
				...
				final JMenuItem[] items =
					buildMenuItems((DefaultMutableTreeNode) root.getChildAt(i));
			...
			final boolean on = animate.getText().equals("Animate");
			animate.setText(on ? "Stop" : "Animate");
			handler.setAnimating(on);
		}
		else if (cmd.equals("animBox")) {
			final int ndx = animBox.getSelectedIndex();
	...
	protected JPanel doDataPanel() {
		...
		final JScrollPane listPane = new JScrollPane(transformList);
		SwingUtil.configureScrollPane(listPane);

		// button for adding a data transform
		final JButton addTransform = new JButton("Add >");
		...
		// burn-in checkbox
		final JCheckBox delayBurn = new JCheckBox("Delay burn-in", true);
		delayBurn.setActionCommand("delayBurn");
		delayBurn.addActionListener(this);
		if (!LAFUtil.isMacLookAndFeel()) delayBurn.setMnemonic('n');
		delayBurn
			.setToolTipText("Toggles whether full-resolution burn-in is delayed");

		// burn-in delay
		delayTime =
			new JSpinner(new SpinnerNumberModel(handler.getBurnDelay() / 1000, 1, 99,
				1));
		delayTime.addChangeListener(this);
		delayTime.setToolTipText("Adjusts the time before full-resolution burn-in");
		...
		// lay out components
		return FormsUtil.makeColumn(FormsUtil.makeRow(new Object[] { listPane,
			buttons, doDataProperties() }, "fill:pref:grow", false), FormsUtil
			.makeRow(delayBurn, delayTime, "seconds"));
	}
	...
	protected JPanel doAnimationPanel() {
		...
		// FPS adjuster
		fps =
			new JSpinner(new SpinnerNumberModel(handler.getAnimationRate(), 1, 60, 1));
		...
		final PanelBuilder builder =
			new PanelBuilder(new FormLayout("pref:grow", "pref, 3dlu, pref, 9dlu"));
		final CellConstraints cc = new CellConstraints();
		builder.add(FormsUtil.makeRow(new Object[] { "Arrow keys:", "Le&ft/right",
			leftRightBox, "Up/do&wn", upDownBox }), cc.xy(1, 1));
		builder.add(FormsUtil
			.makeRow(new Object[] { animate, "&FPS", fps, animBox }), cc.xy(1, 3));
	...
	protected JMenuItem[] buildMenuItems(final DefaultMutableTreeNode node) {
		final DataTransform data = (DataTransform) node.getUserObject();
		final String name = data.toString();
		final int children = node.getChildCount();

		final JMenuItem[] items = new JMenuItem[children == 0 ? 1 : 2];
		...
		for (int i = 0; i < children; i++) {
			...
			final JMenuItem[] jmi =
				buildMenuItems((DefaultMutableTreeNode) node.getChildAt(i));
