...
/**
 * OverlayNodedObject is an overlay object built of nodes likely connected by
 * lines.
 */
...
public abstract class OverlayNodedObject extends OverlayObject {
	...
	private static float[][] arc() {
		final int res = 8; // resolution for 1/8 of circle
	...
	public OverlayNodedObject(final OverlayTransform overlay) {
		super(overlay);
		turnOffHighlighting();
	}

	/** Constructs a noded object. */
	public OverlayNodedObject(final OverlayTransform overlay, final float x1,
	...
	public DataImpl getData() {
		...
		try {
			final float[][] rangeSamples = new float[4][fieldSet.getLength()];
			...
			catch (final VisADException exc2) {
	...
	public double getDistance(final double x, final double y) {
		synchronized (nodesSync) {
			final double[] distSegWt =
				MathUtil.getDistSegWt(nodes, (float) x, (float) y);
			return distSegWt[0];
		}
	}
	...
	protected double[] getNearestNode(final float x, final float y) {
		int minIndex = -1;
		...
		synchronized (nodesSync) {
			for (int i = 0; i < numNodes; i++) {
				final float[] c = { nodes[0][i], nodes[1][i] };
				final double dist = MathUtil.getDistance(c, p);
				if (dist < minDist) {
					minIndex = i;
					minDist = dist;
				}
		...
		return new double[] { minDist, minIndex };
	}
	 ...
	 * point.
	 */
	public double getDistanceToBoundingBox(final double x, final double y) {
		double xdist = 0;
	...
	/** Highlight a node. */
	public void setHighlightNode(final int i) {
	...
	public float[] getNodeCoords(final int index) {
		final float[] retvals = new float[2];
		synchronized (nodesSync) {
	...
	public float[][] getNodes() {
		synchronized (nodesSync) {
			final float[][] copy = new float[2][numNodes];
			for (int i = 0; i < 2; i++) {
	...
	public void setCoords(final float x1, final float y1) {
		...
		final float dx = x1 - this.x1;
		final float dy = y1 - this.y1;
		synchronized (nodesSync) {
			for (int i = 0; i < numNodes; i++) {
	...
	public void setNodes(final float[][] nodes) {
		synchronized (nodesSync) {
			this.nodes = nodes;
			numNodes = nodes[0].length;
			maxNodes = numNodes;
	...
	public double getLastSegmentLength() {
		final float[][] lastSeg =
			{ getNodeCoords(getNumNodes() - 1), getNodeCoords(getNumNodes() - 2) };
		final double[][] lastSegD =
			{ { lastSeg[0][0], lastSeg[0][1] }, { lastSeg[1][0], lastSeg[1][1] } };
		return MathUtil.getDistance(lastSegD[0], lastSegD[1]);
	}

	/** Sets length of curve. */
	public void setCurveLength(final double len) {
		curveLength = len;
	}
	...
	public void setNodeCoords(final int ndx, final float x, final float y) {
		...
		synchronized (nodesSync) {
			if (ndx > 0 && (x == nodes[0][ndx - 1] && y == nodes[1][ndx - 1])) sameAsPrev =
				true;
			if (ndx < numNodes - 1 &&
				(x == nodes[0][ndx + 1] && y == nodes[1][ndx + 1])) sameAsNext = true;
		}
	...
	public void setLastNode(final float x, final float y) {
		setNodeCoords(numNodes - 1, x, y);
	}

	/** Sets coordinates of last node. */
	public void setLastNode(final float[] c) {
		setLastNode(c[0], c[1]);
	}

	/** Sets next node coordinates. */
	public void setNextNode(final float x, final float y) {
		synchronized (nodesSync) {
			if (numNodes > 0 &&
				(x == nodes[0][numNodes - 1] && y == nodes[1][numNodes - 1]))
			{
	...
	/** Sets next node coordinates. */
	public void setNextNode(final float[] c) {
		setNextNode(c[0], c[1]);
	}
	 ...
	 * provided.
	 */
	public void insertNode(final int index, final float x, final float y,
		final boolean colocationalOK)
	{
		synchronized (nodesSync) {
			if (index >= 0 && index < numNodes) {
				...
				if (colocationalOK) {
					...
					final float[] c = new float[] { x, y };
					if (index < numNodes) {
						differentFromNext = MathUtil.areDifferent(getNodeCoords(index), c);
					}
					if (index > 0) {
						differentFromPrev =
							MathUtil.areDifferent(getNodeCoords(index - 1), c);
						notFirst = true;
					}
	...
	private void insert(final int index, final float x, final float y) {
		for (int j = 0; j < 2; j++) {
	...
	public void deleteBetween(final int i1, final int i2) {
		...
		synchronized (nodesSync) {
			if (0 <= i1 && i2 < numNodes && i1 + 1 < i2) {
				...
				// delete one of them
				if (MathUtil.areSame(getNodeCoords(i1), getNodeCoords(i2))) ii2 += 1;
				final int victims = ii2 - i1 - 1;
				final float[][] newNodes = new float[2][maxNodes - victims];
	...
	public void deleteNode(final int index) {
		synchronized (nodesSync) {
			if (index >= 0 && index < numNodes) {
				...
				if (index > 0 && index < numNodes - 1 &&
					MathUtil.areSame(getNodeCoords(index - 1), getNodeCoords(index + 1))) offset =
					1;
				else offset = 0;
				final float[][] newNodes = new float[2][numNodes - 1 - offset];
	 ...
	 * node deleted is an interior node. Returns resulting new freeforms if any.
	 */
	public OverlayFreeform[] removeNode(final int index) {
	 ...
	 */
	public OverlayFreeform connectTo(final OverlayFreeform f2) {
		final float[][] f1Nodes = this.getNodes();
		final float[][] f2Nodes = f2.getNodes();
		final int len1 = f1Nodes[0].length;
		final int len2 = f2Nodes[0].length;

		// Obtain coordinates of last node of f1 and first of f2.
		final float[] f1End = { f1Nodes[0][len1 - 1], f1Nodes[1][len1 - 1] };
		final float[] f2Beg = { f2Nodes[0][0], f2Nodes[1][0] };
		...
		final float[][] newNodes = new float[2][len1 + len2 - offset];
		for (int i = 0; i < 2; i++) {
			System.arraycopy(f1Nodes[i], 0, newNodes[i], 0, len1 - offset);
			System.arraycopy(f2Nodes[i], 0, newNodes[i], len1 - offset, len2);
		}

		final OverlayFreeform f3 = new OverlayFreeform(overlay, newNodes);
		return f3;
	}
	...
	public void reverseNodes() {
		...
		synchronized (nodesSync) {
			final float[][] temp = new float[2][maxNodes];
			for (int j = 0; j < 2; j++) {
	...
	protected void resizeNodeArray(final int newLength) {
		synchronized (nodesSync) {
			final int loopMax = Math.min(nodes[0].length, newLength);
			final float[][] a2 = new float[2][newLength];
	 ...
	 * @param weight the relative distance along the segment to slice The
	 *          parameters seg and weight indicate a 'cut point' on the freeform.
	 *          If weight is strictly between 0 and 1, the cut point is actually
	 *          between two nodes, and all nodes of the original object are
	 *          transferred to the child objects.
	 */
	// TODO -- combine this with 'split' method in polyline tool.
	private OverlayFreeform[] slice(final int seg, final double weight) {
		...
		synchronized (nodesSync) {
			...
			final int numNodes1 = f1Stop + 1;
			final int numNodes2 = f2Stop - f2Start + 1;
	...
	private void printNodes(final float[][] n) {
	...
	public static void printThread(final String methodName) {
