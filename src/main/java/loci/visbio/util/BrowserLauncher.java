...
public final class BrowserLauncher {
	...
	/**
	 * The parameter that needs to be passed into Runtime.exec() to open the
	...
	/**
	 * The shell parameters for Firefox that opens a given URL in an already-open
	 * copy of Firefox on many command-line systems.
	 */
		...
		switch (jvm) {
			case MRJ_2_0:
				try {
					...
					appleEventConstructor =
						appleEventClass.getDeclaredConstructor(new Class[] { int.class,
					...
					putParameter =
						appleEventClass.getDeclaredMethod("putParameter", new Class[] {
							int.class, aeDescClass });
					sendNoReply =
						appleEventClass.getDeclaredMethod("sendNoReply", new Class[0]);

					final Field keyDirectObjectField =
						aeClass.getDeclaredField("keyDirectObject");
					keyDirectObject = (Integer) keyDirectObjectField.get(null);
					final Field autoGenerateReturnIDField =
						appleEventClass.getDeclaredField("kAutoGenerateReturnID");
					kAutoGenerateReturnID = (Integer) autoGenerateReturnIDField.get(null);
					final Field anyTransactionIDField =
			...
			case MRJ_2_1:
				try {
					...
					getFileCreator =
						mrjFileUtilsClass.getDeclaredMethod("getFileCreator",
	...
	private static Object locateBrowser() {
		...
		switch (jvm) {
			case MRJ_2_0:
				try {
					final Integer finderCreatorCode =
						(Integer) makeOSType.invoke(null, new Object[] { FINDER_CREATOR });
					final Object aeTarget =
						aeTargetConstructor.newInstance(new Object[] { finderCreatorCode });
					final Integer gurlType =
						(Integer) makeOSType.invoke(null, new Object[] { GURL_EVENT });
					final Object appleEvent =
						appleEventConstructor.newInstance(new Object[] { gurlType,
							gurlType, aeTarget, kAutoGenerateReturnID, kAnyTransactionID });
		...
		switch (jvm) {
			case MRJ_2_0:
				...
				try {
					...
					putParameter
						.invoke(browser, new Object[] { keyDirectObject, aeDesc });
			...
			case WINDOWS_9X:
				...
				Process process =
					Runtime.getRuntime()
						.exec(
							new String[] { (String) browser, WINDOWS_PARAMETER,
								'"' + url + '"' });
			...
			case OTHER:
				...
				process =
					Runtime
						.getRuntime()
						.exec(
							new String[] { (String) browser, FIREFOX_REMOTE_PARAMETER,
	...
	private static native int ICLaunchURL(int instance, byte[] hint, byte[] data,
